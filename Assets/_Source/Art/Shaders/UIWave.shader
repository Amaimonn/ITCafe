    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (FilterVertexInput v)
            {
                ...
                o.rectIndex = GetFilterRectIndex(v); // Получаем индекс UV региона
                return o;
            }

            // Преобразование UV в нормализованный диапазон [0,1]
            ...
            // Преобразование UV обратно в регион атласа
            ...
            // Пример обработки UV (поворот на 90 градусов)
            float2 RotateUV90(float2 uv)
            {
                return float2(uv.y, 1.0 - uv.x);
            }

            // Пример обработки UV (зеркальное отражение)
